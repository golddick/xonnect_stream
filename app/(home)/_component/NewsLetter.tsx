    ...
    const fetchCategories = async () => {
      try {
        ...
        if (categoryData.length > 0) {
          ...
          setSelectedCategory(first._id)
          setSelectedCategoryName(first.name)
          setSelectedCampaignName(first.campaigns?.[0]?.name || '')
        }
      } catch (error) {
    ...
    try {
      const payload = {
        ...
        metadata: {
          campaign: selectedCampaignName || 'Xonnect website landing page',
          pageUrl: `${process.env.NEXT_PUBLIC_SOURCE}/${selectedCategoryName}`,
          formId: 'xonnct website landing page',
        },
      }
  ...
  return (
    <section className="py-16 px-8 bg-white text-black">
      <div className="max-w-3xl mx-auto text-center">
        <h2 className="text-3xl font-bold mb-6">Subscribe to Our Newsletter</h2>
      <p className="mb-8 text-gray-600">
        Stay informed about upcoming events, the latest uploads, and product launches—delivered directly to your inbox.
        </p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 justify-center">
          <Input
            ...
            required
            className="px-4 py-3 bg-white rounded-md border border-gray-300 w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-transparent"
          />
          <Button
            type="submit"
            disabled={loading}
